	...
	void update(int clock) 
	{
		...
		if (!the_queue.empty()) {
			...
			if ((clock - plane->start_service_time) > plane->service_time) {  
				currentPlaneWait = plane->service_time;
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				landing_queue->total_wait += currentPlaneWait;
				...
				*/
				int rando = my_random.next_int(max_service_time);
				while (rando < min_service_time)
					rando = my_random.next_int(max_service_time);
				plane->service_time = rando;
